...
namespace Windows_Form.Models
{
    class ContactModel
    {
        private static string _selectStatementWithConditionTemplate = "SELECT * FROM contacts WHERE Phones like @keyword";
        public ContactModel()
        {
        ...
        internal object SearchByKeyword(object text)
        {
    ...
    public List<Contact> SearchByKeyword(string keyword)
    {
        ...
        try
        {
            ...
            using (SqliteConnection cnn = new SqliteConnection($"Filename={DatabaseMigration._databasePath}"))
            {
                ...
                while (reader.Read())
                {
                    var phones = Convert.ToString(reader["PhoneNumber"]);
